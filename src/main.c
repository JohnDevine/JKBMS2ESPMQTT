...
void app_main(void) {
    ...
    while (1) {
        ...
        // Feed TWDT during inter-cycle wait so total loop work + delay never exceeds timeout.
        uint32_t remaining_ms = (sample_interval_ms > 0) ? (uint32_t)sample_interval_ms : 1;
        while (remaining_ms > 0) {
            uint32_t chunk_ms = (remaining_ms > 1000) ? 1000 : remaining_ms;
